...
class RegistrationsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should register client user and task_monts should be created as many as month left to ceremony - 13 because one year ahead inluding our month" do
    assert_difference("User.count", 1) do
      post registrations_url, params: {user: { email: @email, password: const_password, password_confirmation: const_password, celebration_date: Time.now + 1.year, role: @clientUser.role} }, as: :json
    end
  ...
  test "should register support user" do
    assert_difference("User.count", 1) do
      post registrations_url, params: {user: { email: @email, password: const_password, password_confirmation: const_password, celebration_date: Time.now + 1.year, role: @supportUser.role} }, as: :json
    end
    body_as_hash = JSON.parse(@response.body, {:symbolize_names=>true})
    assert user_id = body_as_hash[:user][:id]
    newly_created_user = User.find(user_id)
  ...
  test "should not register admin even if in request this admin role is indicated" do
    assert_difference("User.count", 0) do
      post registrations_url, params: {user: { email: @email, password: const_password, password_confirmation: const_password, celebration_date: Time.now + 1.year, role: @adminUser.role} }, as: :json
    end
